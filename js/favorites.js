...
export class FavoritesView extends Favorites {
  ...
  update() {
    ...
    this.entries.forEach(user => {
      ...
      row.querySelector('.card-title h3').textContent = user.name
      row.querySelector('.card-title h4 ').textContent = `${user.login}`
      row.querySelector('.git-profile span').textContent = `${user.login}`
      row.querySelector('.card-body p').textContent = `${user.bio}`
